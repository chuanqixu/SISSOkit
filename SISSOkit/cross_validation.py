...
def kfold(current_path, target_path, property_name, num_fold):
    ...
    data_total = pd.read_csv(os.path.join(
    ...
    batch_size = list(
        map(lambda x: int(math.ceil(x/num_fold)), samples_number))
    ...
    try:
        if os.path.exists(os.path.join(target_path, '%s_cv' % property_name)):
            ...
            if a == 'y':
                shutil.rmtree(os.path.join(
                    target_path, '%s_cv' % property_name))
            if a == 'n':
        ...
        data_total.to_csv(os.path.join(
            target_path, 'train.dat'), index=False, sep=' ')
        with open(os.path.join(target_path, 'cross_validation_info.dat'), 'w') as f:
            json.dump({'cross_validation_type': '%d-fold' % num_fold,
                        'shuffle_data_list': data_list}, f)

    for i in range(0, num_fold):
        try:
            shutil.copytree(current_path, os.path.join(
                target_path, property_name+'_cv%d' % i))
            ...
            if batch_size[task]*i < samples_number[task]:
                for j in range(0, num_fold):
                    if batch_size[task]*j < samples_number[task]:
                        if i == j:
                            val_list.append(
                                data_list[task][batch_size[task]*j:min(batch_size[task]*(j+1), samples_number[task])])
                        else:
                            train_list_t.append(
                                data_list[task][batch_size[task]*j:min(batch_size[task]*(j+1), samples_number[task])])
                    else:
        ...
        with open(os.path.join(target_path, property_name+'_cv%d' % i, 'shuffle.dat'), 'w') as f:
            json.dump({'training_list': train_list, 'training_samples_number': train_len,
                        'validation_list': val_list, 'validation_samples_number': val_len}, f)
        ...
        data_train.to_csv(os.path.join(
            target_path, property_name+'_cv%d' % i, 'train.dat'), index=False, sep=' ')
        data_val.to_csv(os.path.join(target_path, property_name +
                                    '_cv%d' % i, 'validation.dat'), index=False, sep=' ')

        with open(os.path.join(target_path, property_name+'_cv%d' % i, 'SISSO.in'), 'r') as f:
            lines = f.readlines()
            for j in range(len(lines)):
                if lines[j].startswith('nsample'):
                    lines[j] = 'nsample=%s' % (str(train_len).strip(
                        '[]'))+'\t! number of samples for each task (seperate the numbers by comma for ntask >1)\n'
...
def leave_out(current_path, target_path, property_name, num_iter, frac=0, num_out=0):
    ...
    try:
        ...
        with open(os.path.join(target_path, 'cross_validation_info.dat'), 'w') as f:
            if num_out:
                json.dump({'cross_validation_type': 'leave-%d-out' % num_out,
                            'iteration_times': num_iter}, f)
            else:
    ...
    for i in range(0, num_iter):
        ...
        train_list = [[x for x in data_list[task] if x not in val_list[task]]
                        for task in range(0, task_number)]
        train_len = list(map(len, train_list))
        val_len = list(map(len, val_list))
        data_train = data_total.iloc[np.hstack(train_list)-1]
