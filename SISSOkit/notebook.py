...
def generate_report(path, file_path, notebook_name, file_name=None, dim=None):
    r"""
    ...
    if notebook_name == 'regression' or notebook_name == 0:
        ...
        regression = evl.Regression(path[0], dim=dim)
        ...
        if dim != None:
            notebook['cells'][2]['source'][1] = 'regression=evl.Regression(path,dim=%d)\n' % dim
    ...
    if notebook_name == 'regression with CV' or notebook_name == 1:
        ...
        if dim != None:
            notebook['cells'][2]['source'][2] = 'regression=evl.Regression(path,dim=%d)\n' % dim
            notebook['cells'][2]['source'][
                3] = 'regressionCV=evl.RegressionCV(cv_path,dim=%d)\n' % dim
