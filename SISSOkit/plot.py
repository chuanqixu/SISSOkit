...
def baselineplot(regression, unit=None, marker='x', marker_color='r', marker_y=1, marker_shape=100, fontsize=20, **kw):
    ...
    plt.scatter(regression.property.mean(), marker_y, marker=marker,
                c=marker_color, s=marker_shape, zorder=1, label='Mean Value')
    if unit:
    ...
    plt.title('Histogram of %s over training data\nMean = %.5f\nStandard deviation = %.5f' % (
        regression.property_name, regression.property.mean(), regression.property.std()), fontsize=fontsize)
    plt.legend()
...
def error_hist(dimension, *regressions, training=True, absolute=False, unit=None, fontsize=20, **kw):
    ...
    dimension -= 1
    if absolute:
        collect_data = np.hstack([np.abs(regression.errors(training=training))[
                                 dimension, :] for regression in regressions])
        if unit:
            plt.xlabel('Absolute error %s' % ('['+unit+']'), fontsize=fontsize)
        else:
            plt.xlabel('Absolute error', fontsize=fontsize)
    else:
        collect_data = np.hstack([regression.errors(training=training)[
                                 dimension, :] for regression in regressions])
        if unit:
...
def prediction_vs_property(dimension, *regressions, training=True, unit=None, fontsize=20, **kw):
    ...
    if isinstance(regressions[0], evl.RegressionCV):
        if training:
            property_values = np.hstack(
                [regression.property[cv].values for regression in regressions for cv in range(0, regression.n_cv)])
        else:
            property_values = np.hstack([regression.validation_data[cv].iloc[:, 1].values.tolist(
            ) for regression in regressions for cv in range(0, regression.n_cv)])
        prediction_value = np.hstack([regression.predictions(training=training)[
                                     dimension, :] for regression in regressions])-property_values
    else:
        property_values = np.hstack(
            [regression.property.values for regression in regressions])
        prediction_value = np.hstack([regression.predictions(training=training)[
                                     dimension, :] for regression in regressions])-property_values
    plt.scatter(property_values, prediction_value, **kw)
    if unit:
        plt.xlabel('%s in data set %s' % (
            regressions[0].property_name, '['+unit+']'), fontsize=fontsize)
        plt.ylabel('Signed error %s' % ('['+unit+']'), fontsize=fontsize)
    else:
        plt.xlabel('%s in data set' %
                   (regressions[0].property_name), fontsize=fontsize)
...
def hist_with_markers(dimension, *regressions, training=True, unit=None, fontsize=20, selected_errors=None, marker_x=0, marker=None, **kw):
    ...
    if selected_errors == None:
        selected_errors = ('RMSE', 'MAE', '25%ile AE',
                           '50%ile AE', '75%ile AE', '95%ile AE', 'MaxAE')
    if marker == None:
        marker = {'RMSE': 's', 'MAE': 'x', '25%ile AE': 'p',
                  '50%ile AE': 'X', '75%ile AE': 'D', '95%ile AE': '+', 'MaxAE': '.'}
    for selected_error in selected_errors:
        plt.scatter(marker_x, errors[selected_error], s=50, zorder=1,
                    marker=marker[selected_error], label=selected_error)
    plt.legend()
...
def abs_errors_vs_dimension(*regressions, training=True, unit=None, fontsize=20, selected_errors=None, display_baseline=False, label='', **kw):
    ...
    if display_baseline:
        plt.plot([1, regressions[0].dimension], [
                 regressions[0].baseline[1], regressions[0].baseline[1]], '--', label='Baseline')
    collect_data = np.hstack(
        [np.abs(regression.errors(training=training)) for regression in regressions])
    errors = evl.compute_errors(collect_data)
    if selected_errors == None:
        selected_errors = ('RMSE', 'MAE', '25%ile AE',
                           '50%ile AE', '75%ile AE', '95%ile AE', 'MaxAE')
...
def errors_details(regression, training=True):
    ...
    for i in range(1, regression.dimension+1):
        ...
        error_hist(i, regression, absolute=False,
                   training=training, rwidth=0.8)
        ...
        hist_with_markers(i, regression, training=training,
                          bins=20, alpha=0.5, rwidth=0.8, marker_x=10)
