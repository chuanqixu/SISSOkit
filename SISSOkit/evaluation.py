...
def evaluate_expression(expression, data):
    ...
    operators = ['+', '-', '*', '/', 'exp', 'exp-', '^-1', '^2', '^3',
                 'sqrt', 'cbrt', 'log', 'abs', '^6', 'sin', 'cos', '(', ')']
    OPTR = []
    OPND = []
    s = ''
    i = 0
    ...
    if data:
        ...
        if dimension_idx:
            for i in range(dimension_idx):
                value += result.coefficients[task_idx-1][dimension_idx-1][i] * \
                    evaluate_expression(
                        result.descriptors[dimension_idx-1][i], data)
            value += result.intercepts[task_idx-1][dimension_idx-1]
            pred = value
        else:
            for dimension in range(result.dimension):
                for i in range(dimension):
                    value += result.coefficients[task_idx-1][dimension][i] * \
                        evaluate_expression(
                            result.descriptors[dimension][i], data)
        ...
        if training:
            for task in range(result.n_task):
                ...
                for dimension in range(result.dimension):
                    ...
                    for i in range(dimension+1):
                        value += result.coefficients[task][dimension][i]*evaluate_expression(
                            result.descriptors[dimension][i], result.data_task[task])
            ...
            for task in range(result.n_task):
                ...
                for dimension in range(result.dimension):
                    ...
                    for i in range(dimension+1):
                        value += result.coefficients[task][dimension][i]*evaluate_expression(
                            result.descriptors[dimension][i], result.validation_data_task[task])
                    value += result.intercepts[task][dimension]
                    pred_t.append(list(value))
                pred.append(np.array(pred_t))
    return pred
    ...
    if (isinstance(intercepts, Iterable) and isinstance(intercepts[0], Iterable)):
        ...
        for task in tasks:
            ...
            for dimension in dimensions:
                for i in range(dimension):
                    pred[d][sample_idxs] += coefficients[task_idx][dimension-1][i] * \
                        evaluate_expression(
                            descriptors[dimension-1][i], data.iloc[sample_idxs])
...
def compute_errors(errors):
    ...
    # 2D [dimension, sample]
        ...
        error = pd.DataFrame([compute_errors(errors[dimension]) for dimension in range(n_dimension)],
                             columns=['RMSE', 'MAE', '25%ile AE', '50%ile AE',
                                      '75%ile AE', '95%ile AE', 'MaxAE'],
                             index=list(range(1, n_dimension+1)))
...
class Regression(object):
    ...
    def __init__(self, current_path, dim=None):
        ...
        with open(os.path.join(self.current_path, 'SISSO.in'), 'r') as f:
            ...
            n_sample = list(
                filter(lambda x: bool(re.search(r'\d', x)), n_sample))
            # if n_sample[-1]=='':
            #    n_sample=n_sample[:-1]
            n_sample = list(map(int, n_sample))
            task_weighting = int(re.findall(
                r'task_weighting\s*=\s*(\d+)', input_file)[0])
            ...
            maxcomplexity = int(re.findall(
                r'maxcomplexity\s*=\s*(\d+)', input_file)[0])
            dimclass = re.findall(r'dimclass=([\(\d:\)]+)', input_file)[0]
            maxfval_lb = float(re.findall(
                r'maxfval_lb\s*=\s*(\d+.\d+|\d+[eE]-?\d+)', input_file)[0])
            maxfval_ub = float(re.findall(
                r'maxfval_ub\s*=\s*(\d+.\d+|\d+[eE]-?\d+)', input_file)[0])
            ...
            L1L0_size4LO = int(re.findall(
                r'L1L0_size4L0\s*=\s*(\d+)', input_file)[0])
            fit_intercept = re.findall(
                r'fit_intercept\s*=\s*.(\w+).', input_file)[0]
            if fit_intercept == 'true':
                fit_intercept = True
            else:
                fit_intercept = False
            metric = re.findall(r'metric\s*=\s*\'(\w+)', input_file)[0]
            nm_output = int(re.findall(
                r'nm_output\s*=\s*(\d+)', input_file)[0])
        ...
        self.data = pd.read_csv(os.path.join(
            current_path, 'train.dat'), sep=r'\s+')
        ...
        self.materials_task = utils.seperate_DataFrame(
            self.materials, self.n_sample)
        self.property_task = utils.seperate_DataFrame(
            self.property, self.n_sample)

        # ------------------------- read validation data ---------------------------

        if os.path.exists(os.path.join(self.current_path, 'validation.dat')):
            self.validation_data = pd.read_csv(os.path.join(
                current_path, 'validation.dat'), sep=r'\s+')
            ...
            self.validation_data_task = utils.seperate_DataFrame(
                self.validation_data, self.n_validation_sample)
    ...
    def get_descriptors(self, path=None):
        ...
        with open(os.path.join(path, 'SISSO.out'), 'r') as f:
            input_file = f.read()
            descriptors_total = re.findall(
                r'descriptor:[\s\S]*?coefficients', input_file)
            for dimension in range(0, self.dimension):
                ...
                descriptors_d = [x[1] for x in list(
                    map(lambda x: re.split(r':', x), descriptors_d))]
                descriptors_d = list(
                    map(lambda x: x.replace(r'[', r'('), descriptors_d))
                descriptors_d = list(
                    map(lambda x: x.replace(r']', r')'), descriptors_d))
    ...
    def get_coefficients(self, path=None):
        ...
        for task in range(0, self.n_task):
            ...
            with open(os.path.join(path, 'SISSO.out'), 'r') as f:
                input_file = f.read()
                coefficients_total = re.findall(
                    r'coefficients_00%d:(.*)' % (task+1), input_file)
                for dimension in range(self.dimension):
                    coefficients_d = re.split(
                        r'\s+', coefficients_total[dimension])[1:]
    ...
    def get_intercepts(self, path=None):
        ...
        for task in range(0, self.n_task):
            with open(os.path.join(path, 'SISSO.out'), 'r') as f:
                input_file = f.read()
                intercepts_t = re.findall(
                    r'Intercept_00%d:(.*)' % (task+1), input_file)
    ...
    def features_percent(self, descending=True):
        ...
        feature_space = pd.read_csv(os.path.join(
            self.current_path, 'feature_space', 'Uspace.name'), sep=r'\s+', header=None).iloc[0:self.subs_sis, 0]
        feature_percent = pd.DataFrame(
            columns=self.features_name, index=('percent',))
        for feature_name in self.features_name:
            percent = feature_space.str.contains(
                feature_name).sum()/self.subs_sis
            feature_percent.loc['percent', feature_name] = percent
        feature_percent = feature_percent.T
        if descending:
            feature_percent.sort_values(
                'percent', ascending=False, inplace=True)
    ...
    def check_predictions(self, dimension, multiply_coefficients=True):
        ...
        for task in range(len(n_sample)):
            predictions[0, total_n_sample:total_n_sample +
                        n_sample[task]] = self.intercepts[task][dimension-1]
            total_n_sample += n_sample[task]
        if multiply_coefficients:
            for d in range(dimension):
                total_n_sample = 0
                for task in range(len(n_sample)):
                    predictions[d+1, total_n_sample:total_n_sample+n_sample[task]] = coefficients[task][dimension -
                                                                                                        1][d]*evaluate_expression(descriptors[dimension-1][d], data[task]).values
                    total_n_sample += n_sample[task]
        else:
            for d in range(dimension):
                total_n_sample = 0
                for task in range(len(n_sample)):
                    predictions[d+1, total_n_sample:total_n_sample+n_sample[task]
                                ] = evaluate_expression(descriptors[dimension-1][d], data[task]).values
                    total_n_sample += n_sample[task]
        return predictions
...
class RegressionCV(Regression):
    ...
    def __init__(self, current_path, property_name=None, drop_index=[], dim=None):
        # -------------------------- read path and CV info----------------------------
        cv_names = sorted(list(filter(lambda x: '_cv' in x, os.listdir(current_path))),
                          key=lambda x: int(x.split('_cv')[-1]))
        dir_list = list(map(lambda cv_name: os.path.join(
            current_path, cv_name), cv_names))
        n_cv = len(cv_names)

        self.current_path = current_path
        self.cv_path = [dir_list[cv]
                        for cv in range(n_cv) if cv not in drop_index]
        ...
        self.total_data = pd.read_csv(os.path.join(
            current_path, 'train.dat'), sep=r'\s+')
        self.n_total_sample = len(pd.read_csv(
            os.path.join(current_path, 'train.dat'), sep=r'\s+'))
        self.property_name = property_name if property_name else cv_names[0].split('_cv')[
            0]

        # -------------------------- read data per CV -----------------------------

        self.data = [pd.read_csv(os.path.join(
            cv_path, 'train.dat'), sep=r'\s+') for cv_path in self.cv_path]
        ...
        self.n_validation_sample = np.array([json.load(open(os.path.join(cv_path, 'shuffle.dat'), 'r'))['validation_samples_number']
                                             for cv_path in self.cv_path])
        self.validation_data = [pd.read_csv(os.path.join(
            cv_path, 'validation.dat'), sep=r'\s+') for cv_path in self.cv_path]
    ...
    def __getitem__(self, index):
        if isinstance(index, slice):
            return [Regression(self.cv_path[i], dim=self.dimension) for i in range(index.start, index.stop)]
        else:
            return Regression(self.cv_path[index], dim=self.dimension)
    ...
    def __repr__(self):
        ...
        if 'shuffle_data_list' in cv_info:
            text += ('\nCross Validation Type: %s\nShuffle Data List: ' %
                     cv_info['cross_validation_type']+str(cv_info['shuffle_data_list']))
        else:
            text += ('\nCross Validation Type: %s\nIteration: ' %
                     cv_info['cross_validation_type']+str(self.n_cv))
        text += '\nProperty Name: %s\nTask Number: %d\nRung: %d\nDimension: %d\nSubs_sis: %d' % (
            self.property_name, self.n_task, self.rung, self.dimension, self.subs_sis)
        return text
    ...
    def features_percent(self, descending=True):
        ...
        for cv_path in self.cv_path:
            feature_space = pd.read_csv(os.path.join(
                cv_path, 'feature_space', 'Uspace.name'), sep=r'\s+', header=None).iloc[0:self.subs_sis, 0]
            for feature_name in self.features_name:
                count = feature_space.str.contains(feature_name).sum()
                feature_percent.loc['percent', feature_name] += count
        feature_percent.iloc[0, :] = feature_percent.iloc[0,
                                                          :]/(self.n_cv*self.subs_sis)
        feature_percent = feature_percent.T
        if descending:
            feature_percent.sort_values(
                'percent', ascending=False, inplace=True)
    ...
    def descriptor_percent(self, descriptor):
        ...
        for cv in range(0, self.n_cv):
            feature_space = pd.read_csv(os.path.join(
                self.cv_path[cv], 'feature_space', 'Uspace.name'), sep=r'\s+', header=None).iloc[0:self.subs_sis, 0]
            try:
                descriptor_index[cv] = feature_space.tolist().index(
                    descriptor)+1
                count += 1
            except ValueError:
    ...
    def predictions(self, training=True, display_cv=False, display_task=False):
        ...
        if display_cv == True:
            if display_task == True:
                return [compute_using_model_reg(result=self[i],
                                                training=training)
                        for i in range(self.n_cv)]
            else:
                return [np.hstack(compute_using_model_reg(result=self[i],
                                                          training=training))
                        for i in range(self.n_cv)]
        else:
    ...
    def errors(self, training=True, display_cv=False, display_task=False):
        ...
        if display_cv:
            if training:
                if display_task:
                    ...
                    pred = self.predictions(
                        training=True, display_cv=True, display_task=True)
                    for cv in range(self.n_cv):
                        error_cv = []
                        for task in range(0, self.n_task):
                            error_cv.append(
                                pred[cv][task]-self.property_task[cv][task])
                        error.append(error_cv)
                    return error
                else:
                    pred = self.predictions(
                ...
                if display_task:
                    error = []
                    pred = self.predictions(
                        training=False, display_cv=True, display_task=True)
                    for cv in range(0, self.n_cv):
                        error_cv = []
                        for task in range(self.n_task):
                            error_cv.append(
                                pred[cv][task]-self.validation_data_task[cv][task].iloc[:, 1].values)
                        error.append(error_cv)
                    return error
                else:
                    pred = self.predictions(
            ...
            if display_task:
                errors_cv_t = self.errors(
                    training=training, display_cv=True, display_task=True)
                errors = []
                for task in range(self.n_task):
    ...
    def drop(self, index=[]):
        ...
        return RegressionCV(self.current_path, self.property_name, index, dim=self.dimension)
    ...
    def check_predictions(self, cv_idx, dimension, training=True, multiply_coefficients=True):
        ...
        total_n_sample = 0
        for task in range(len(n_sample)):
            predictions[0, total_n_sample:total_n_sample+n_sample[task]
                        ] = self.intercepts[cv_idx][task][dimension-1]
            total_n_sample += n_sample[task]
        if multiply_coefficients:
            for d in range(dimension):
                total_n_sample = 0
...
class Classification(object):
    ...
    def __init__(self, current_path):
        ...
        with open(os.path.join(self.current_path, 'SISSO.in'), 'r') as f:
            ...
            n_task = int(re.findall(r'ntask\s*=\s*(\d+)', input_file)[0])
            n_sample = re.findall(
                r'nsample\s*=\s*([\(\)\d,\s]*)\)', input_file)[0]
            n_sample = re.split(r'\(|\s', n_sample)
            n_sample = list(
                filter(lambda x: bool(re.search(r'\d', x)), n_sample))
            n_sample = [list(map(lambda d: int(d), filter(lambda x: bool(
                re.search(r'\d', x)), re.split(r',|\s|\)', n)))) for n in n_sample]
            dimension = int(re.findall(r'desc_dim\s*=\s*(\d+)', input_file)[0])
            ...
            method = re.findall(r'method\s*=\s*\'(\w+)', input_file)[0]
            isconvex = re.findall(
                r'isconvex\s*=\s*([\(\)\d,\s]*)\)', input_file)[0]
            isconvex = re.split(r'\(|\s', isconvex)
            isconvex = list(
                filter(lambda x: bool(re.search(r'\d', x)), isconvex))
            isconvex = [list(map(lambda d: int(d), filter(lambda x: bool(
                re.search(r'\d', x)), re.split(r',|\s|\)', n)))) for n in isconvex]
            width = float(re.findall(
                r'width\s*=\s*(\d+.\d+|\d+[eE]-?\d+)', input_file)[0])
            nm_output = int(re.findall(
                r'nm_output\s*=\s*(\d+)', input_file)[0])
        ...
        self.materials_sep = utils.seperate_DataFrame(
            self.materials, self.n_sample)
    ...
    def __repr__(self):
        ...
        if 'shuffle_data_list' in cv_info:
            ...
            text += ('\nCross Validation Type: %s\nIteration: ' %
                     cv_info['cross_validation_type']+str(self.n_cv))
        text += '\nTask Number: %d\nRung: %d\nDimension: %d\nSubs_sis: %d' % (
            self.n_task, self.rung, self.dimension, self.subs_sis)
        return text
    '''
    @property
    def baseline(self):
