...
def evaluate_expression(expression,data):
    ...
    Arguments:
        ...
        data (pandas.DataFrame): the data you want to compute, with samples as index and features as columns.
...
def compute_using_model(path=None,result=None,training=True,data=None,task_idx=None,dimension_idx=None):
    r"""
    ...
    Arguments:
        path (string): directory path of SISSO result which contains the model you want to use.
        
        result (Regression): instance of Regression which contains the model you want to use.
        
        training (bool): whether the task is training or predicting. Default is True.
        
        data (string or pandas.DataFrame): the path of the data or the data you want to compute. 
        If it is :class:`string`, it will be recognized as path to the data and 
        use ``data=pd.read_csv(data,sep=' ')`` to read the data, so remember to use space to seperate the data.
        Otherwise it should be :class:`pandas.DataFrame`.
        
        task_idx (integer): specifies which task of model you want to use.
        
        dimension_idx (integer): specifies which dimension of model you want to use.
...
def predict(data,descriptors,coefficients,intercepts,tasks=None,dimensions=None):
    ...
    Arguments:
        ...
        descriptors (list): the descriptors you want to use. The index order should be dimension of model then dimension of desciptor.
        
        coefficients (list): the coefficients you want to use. The index order should be task, dimension of model then dimension of desciptor.
        
        intercepts (float or list): the intercepts you want to use. The index order should be task then dimension of model.
        
        tasks (None or list): specifies which sample should use which task of model to compute.
        ...
        dimensions (None or list): specifies which dimension of desctiptor will be used.
...
def compute_errors(errors):
    ...
    Arguments:
        errors: difference between predictions and exact value.
...
class Regression(object):
    ...
    Arguments:
        current_path (string): path to the directory of the SISSO result.
    ...
    def predictions(self,training=True,display_task=False):
        ...
        Arguments:
            training (bool): determines whether its training or not.
            
            display_task (bool): determines the predictions contain index of task or not.
    ...
    def errors(self,training=True,display_task=False):
        ...
        Arguments:
            ...
            display_task (bool): determines the errors contain index of task or not.
    ...
    def total_errors(self,training=True,display_task=False,display_baseline=False):
        ...
        Arguments:
            ...
            display_baseline (bool): determines whether display baseline
...
class RegressionCV(Regression):
    ...
    Arguments:
        current_path (string): path to the directory of the cross validation results.
        
        property_name (string): specifies the property name of your CV results.
        
        drop_index (list): specifies which CV results you don't want to consider.
    ...
    def descriptor_percent(self,descriptor):
        ...
        Arguments:
            descriptor (string): the descriptor you want to check.
        """
    ...
    def predict(self,data,cv_index=None,tasks=None,dimensions=None):
        ...
        Arguments:
            ...
            cv_index (None or list): specifies which CV should be included.
    ...
    def predictions(self,training=True,display_cv=False,display_task=False):
        ...
        if display_cv==True:
            if display_task==True:
                return [compute_using_model(path=cv_path,
                                        training=training)
                for cv_path in self.cv_path]
            else:
                return [np.hstack(compute_using_model(path=cv_path,
                                        training=training))
                for cv_path in self.cv_path]
        else:
    ...
    def total_errors(self,training=True,display_cv=False,display_task=False,display_baseline=False):
        ...
        Arguments:
            ...
            display_cv (bool): determines the errors contain index of CV or not.
    ...
    def drop(self,index=[]):
        ...
        Arguments:
            index (list): contain the CV index which you want to drop.
    ...
    def check_predictions(self,cv_idx,dimension,training=True,multiply_coefficients=True):
        ...
        Arguments:
            cv_idx (integer): specifies which CV result you want to check.
            
            training (bool): whether it is training.
            
            multiply_coefficients (bool): whether it should be multiplied by coefficients.
